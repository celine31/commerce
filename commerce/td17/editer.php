...
// Arriver depuis editer.php pour INSERT ou UPDATE
    ...
    if (!$tabErreur) {
        ...
        if ($produit->id_produit) {
            $req = "UPDATE produit SET id_categorie={$produit->id_categorie},nom={$cnx->esc($produit->nom)},ref={$cnx->esc($produit->ref)},prix={$produit->prix} WHERE id_produit = {$produit->id_produit}";
            $cnx->xeq($req);
        } else {
            $req = "INSERT INTO produit VALUES(DEFAULT,{$produit->id_categorie}, {$cnx->esc($produit->nom)},{$cnx->esc($produit->ref)},{$produit->prix})";

            $cnx->xeq($req);
            $produit->id_produit = $cnx->pk();
        }
    ...
    if (!$cnx->xeq($req)->ins($produit)) {                     // si le jeu ramène false on ramene vers page indispo(car piratage)
